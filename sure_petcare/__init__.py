...
_URL_CONTROL = _URL_DEV + '/%s/control'
...
class SurePetFlapAPI(object):
    ...
    def _set_data(self, url, data):
        ...
        if url in self.cache:
            ...
            headers = self._create_header()
        if headers is not None:
            response = self._api_set(url, headers, data)
            if response.status_code in [304, 404, 500, 502, 503, 504, ]:
                # Used cached data in event of (respectively), not modified,
                # server error, server overload, server unavailable and gateway
                # timeout.  Doesn't cope with such events absent cached data,
    ...
    def _api_set(self, url, headers, data):
        r = self.s.put(url, headers=headers, json=data)
        if r.status_code == 401:
            # Retry once
            self.update_authtoken(force=True)
            if 'Authorization' in headers['headers']:
                headers['headers']['Authorization'] = 'Bearer ' + self.cache['AuthToken']
                r = self.s.put(url, headers=headers, json=data)
            else:
                raise SPAPIException('Auth required but not present in header')
        return r
...
class SurePetFlapMixin( object ):
    ...
    def set_enable_curfew(self, enable, lock_time=None, unlock_time=None, household_id=None, flap_id=None):
        """
        Either enables or disables a curfew on the given flap.
        :param enable: True to enable, False to disable
        :param lock_time: the optional lock time
        :param unlock_time: the optional unlock time
        :param flap_id: the flap ID; will be default if not specified
        :param household_id: the household ID; will be default if not specified
        """
